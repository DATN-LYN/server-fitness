...
export class AuthService {
  ...
  async register(input: RegisterInputDto) {
    const { email, password, avatar, age, fullName, userRole, gender } = input;
    const user = await UserCommand.findByEmail(email);
    if (user) {
    ...
    await getManager().transaction(
      async (transaction) =>
        await transaction
          .getRepository(User)
          .create({ email: email, avatar: avatar, age: age, password: hashPassword, fullName: fullName, userRole: userRole, gender: gender })
          .save(),
    );
    return {
      message: 'oke',
